...
projects/alchemy/apps/website/src/content/docs/providers/{Cloud}/{Resource}.md  # API reference, generated by `bun run docs:gen`
```
...
1. **Catalog** — fan out research agents over the provider's distilled service modules (one batch per thematic group). Each agent reads the generated SDK (`projects/distilled/packages/{cloud}/src/services/{service}.ts`), cross-references the vendor API docs, and writes a self-contained design spec to `docs/provider-catalog/{Cloud}/catalog/{service}.md`: resources, namespaces, props/attrs with replacement rules, lifecycle-to-operation mapping, scope (account/zone), testability, priority. The coordinator aggregates a machine-readable `summary.json` + human `INDEX.md` that tracks `implemented | partial | missing` per resource — this is the factory's order book.
...
```sh
bun run docs:gen   # -> projects/alchemy/apps/website/src/content/docs/providers/{Cloud}/{Resource}.md
```
...
After editing JSDoc on a resource, run `bun run docs:gen` to refresh the website docs.
...
11. Add the resource-level JSDoc (`@section` + `@example` blocks) and field-level JSDoc on each prop/attribute on the source `.ts` file. Then run `bun run docs:gen` to refresh `projects/alchemy/apps/website/src/content/docs/providers/{Cloud}/{Resource}.md`. Do NOT manually edit the generated markdown.
