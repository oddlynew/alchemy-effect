...
| Website cache inputs | Override `@oddlynew/alchemy-website` `build` and `docs:check` inputs with `websiteSources`. | The global `production` input intentionally ignores Markdown for library builds, but the website output depends on MD/MDX docs and generated provider docs. `websiteSources` includes content while excluding generated docs, `.astro`, `dist`, and downloaded fonts from the hash. |
