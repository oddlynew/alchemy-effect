...
const REPO = "alchemy-run/cloudflare-tools";
...
async function checkGithubTag(version: string): Promise<boolean> {
  try {
    const response = await fetch(`https://api.github.com/repos/${REPO}/git/refs/tags/v${version}`);
    return response.ok;
  } catch {
    return false;
  }
...
async function getPackageDirs(): Promise<Array<string>> {
...
if (!versionInput) {
  console.error("Please provide a version number or bump type (major, minor, patch)");
  process.exit(1);
}

// Read current version from cloudflare-rolldown-plugin as the source of truth
// (it's at the root of the workspace dep chain; all packages share one version).
const rootPackageJsonPath = join(PACKAGES_DIR, "cloudflare-rolldown-plugin", "package.json");
...
if (["major", "minor", "patch"].includes(versionInput)) {
  ...
  console.log(`Bumping ${versionInput} version: ${currentVersion} -> ${newVersion}`);
} else {
  if (!/^\d+\.\d+\.\d+(-[\w.]+)?$/.test(versionInput)) {
    console.error("Version must be in format x.y.z or x.y.z-pre or use 'major', 'minor', 'patch'");
    process.exit(1);
  }
...
// Check if version already exists on npm (check rolldown plugin as representative)
const npmExists = await checkNpmVersion(rootPackageJson.name, newVersion);
if (npmExists) {
...
for (const dir of packageDirs) {
  ...
  if (pkgJson.private) {
    console.log(`  Skipped ${pkgJson.name} (private)`);
    continue;
  }
...
function generateChangelogEntry(version: string): string {
  ...
  for (const line of logOutput.split("\n")) {
    ...
    if (!subject.startsWith("feat") && !subject.startsWith("fix") && !subject.startsWith("perf"))
      continue;
    ...
    const descClean = prMatch ? description.slice(0, prMatch.index).trim() : description.trim();
