...
describe("resolveCompatibilityOptions", () => {
  ...
  test("it opts you into new flags if you have a future date", ({ expect }) => {
    const { compatibilityDate, compatibilityFlags } = resolveCompatibilityOptions({
    ...
    expect(compatibilityFlags).toContain("assets_navigation_prefers_asset_serving");
  });
  ...
  test("it doesn't double you up if you've already opted into new flags and have a future date", ({
    ...
    const { compatibilityDate, compatibilityFlags } = resolveCompatibilityOptions({
      compatibility_flags: ["foo", "bar", "assets_navigation_prefers_asset_serving"],
      compatibility_date: "2099-12-12",
    });
    ...
    expect(compatibilityFlags).toContain("bar");
    expect(
      compatibilityFlags.filter((flag) => flag === "assets_navigation_prefers_asset_serving")
        .length,
    ).toBe(1);
  });
  ...
  test("it doesn't opt you into new flags if you have a future date and have disabled it", ({
    ...
    expect(compatibilityFlags).toContain("foo");
    expect(compatibilityFlags).toContain("bar");
    expect(compatibilityFlags).not.toContain("assets_navigation_prefers_asset_serving");
  });
});
