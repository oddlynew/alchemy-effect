...
import { listParameters } from "../src/operations/listParameters";
import {
  getTestDatabase,
  runEffect,
  setupTestDatabase,
...
describe("parameters", () => {
  ...
  describe("listParameters", () => {
    it("can list parameters (or returns error if not available for database type)", async () => {
      const db = getDb();
      ...
      // Parameters may not be available for all database types (MySQL vs PostgreSQL)
      if ("data" in result) {
        ...
        if (result.data.length > 0) {
          ...
          expect(param.display_name).toBeDefined();
