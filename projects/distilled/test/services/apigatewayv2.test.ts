...
test(
  ...
  withApi("itty-apigwv2-route-integration", "HTTP", (apiId) =>
    Effect.gen(function* () {
      ...
      expect(route.RouteId).toBeDefined();
      expect(route.Target).toEqual(`integrations/${integration.IntegrationId}`);
  ...
  withApi("itty-apigwv2-autodeploy", "HTTP", (apiId) =>
    Effect.gen(function* () {
      ...
      return yield* Effect.gen(function* () {
        ...
        Effect.ensuring(
          deleteStage({ ApiId: apiId, StageName: "prod" }).pipe(Effect.ignore),
        ),
      );
    }),
  ),
...
test(
  ...
  withApi("itty-apigwv2-full-lifecycle", "HTTP", (apiId) =>
    Effect.gen(function* () {
      ...
      // Clean up
      yield* deleteStage({ ApiId: apiId, StageName: "v1" }).pipe(Effect.ignore);
      yield* deleteDeployment({
        ApiId: apiId,
        DeploymentId: deployment.DeploymentId!,
      }).pipe(Effect.ignore);
...
test(
  ...
  withApi("itty-apigwv2-websocket", "WEBSOCKET", (apiId) =>
    Effect.gen(function* () {
      ...
      yield* deleteRoute({ ApiId: apiId, RouteId: defaultRoute.RouteId! }).pipe(
        Effect.ignore,
      );
      yield* deleteRoute({
        ApiId: apiId,
