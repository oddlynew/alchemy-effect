...
import { FeatureFlagsControllerEnableFlag } from "../src/operations/FeatureFlagsControllerEnableFlag.ts";
import { FeatureFlagsControllerList } from "../src/operations/FeatureFlagsControllerList.ts";
import { runEffect, testRunId } from "./setup.ts";

describe("FeatureFlagsControllerEnableFlag", () => {
  it("enables a feature flag", { timeout: 30_000 }, async () => {
    ...
    if (list.data.length === 0) {
      ...
      const error = await runEffect(
        FeatureFlagsControllerEnableFlag({
    ...
    const flag = await runEffect(
      FeatureFlagsControllerEnableFlag({ slug: seed.slug }),
    );
