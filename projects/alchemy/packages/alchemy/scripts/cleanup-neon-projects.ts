 ...
 *
 * Usage:
 *   doppler run -c dev --project alchemy-effect-fork -- bun projects/alchemy/packages/alchemy/scripts/cleanup-neon-projects.ts
 *   DRY_RUN=1 doppler run -c dev --project alchemy-effect-fork -- bun projects/alchemy/packages/alchemy/scripts/cleanup-neon-projects.ts
 *   CONCURRENCY=4 doppler run -c dev --project alchemy-effect-fork -- bun projects/alchemy/packages/alchemy/scripts/cleanup-neon-projects.ts
 *
 * Environment:
 *   NEON_API_KEY   Required. Injected by Doppler (`alchemy-effect-fork` / `dev`) or `.env` via `bun download:env`.
