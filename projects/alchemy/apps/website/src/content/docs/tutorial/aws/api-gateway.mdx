...
The repo includes **`projects/alchemy/examples/aws-rest-api`**: a minimal **regional** REST API with `ANY` on the root and a `{proxy+}` resource, **AWS_PROXY** integration to a Lambda, then **Deployment**, **Stage** (`prod`), and **Lambda.Permission** so API Gateway can invoke the function.

```sh
cd projects/alchemy/examples/aws-rest-api
bun install
bun run deploy
```
...
Generated docs for each primitive live under **Providers → AWS → ApiGateway** in the sidebar (for example [RestApi](/providers/aws/apigateway/restapi)). Run `bun nx build:reference @oddlynew/alchemy-website` locally after changing JSDoc on resources.
