 ...
 * declared via `env: { FLAGS: Cloudflare.Flagship(...) }`. `InferEnv` maps the
 * marker to the native `Flagship` runtime binding, so the handler calls
 * `env.FLAGS.getBooleanValue(...)` directly.
 */
export default {
