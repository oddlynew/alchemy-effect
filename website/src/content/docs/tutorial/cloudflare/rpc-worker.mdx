...
## Bonus: scaffold a caller Worker

Once the single-worker flow is solid, a second `RpcWorker` can call
the first via `Cloudflare.RpcWorker.bind`. Start with a bare class:
...
Same `RpcWorker` shape as before, with its own schema (`CallerRpcs`).

## Register the binding at init
...
the service binding on this Worker so workerd surfaces the stub on
`env`, and returns an `Effect<RpcClient>` *factory* the handlers
will use.
...
Each per-request handler yields the factory to build a fresh
`RpcClient`, then calls `tasks.getTask({...})` over the in-account
service binding. The call is fully typed against `TaskWorker`'s
...
Same return shape as the original Worker — the caller is itself an
`RpcWorker` whose `fetch` surface is its own `RpcGroup`.
